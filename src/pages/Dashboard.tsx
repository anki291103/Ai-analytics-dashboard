...
const Dashboard: React.FC = () => {
  ...
  const [liveSalesData, setLiveSalesData] = useState<SalesDataItem[]>(initialSalesData);
  ...
  // Simulate data loading (initial load or filter/view change)
  useEffect(() => {
    setLoading(true);
  ...
  }, [activeView, dateRange, category]); // Recalculate/show loading on view or filter change
  ...
  const handleNaturalLanguageSearch = (query: string) => {
    ...
    } else if (parsed.action === 'display_insight') {
      setNlpGeneratedInsight(parsed.responseInsight);
    }
    setSelectedDrillDownData(undefined);
    setIsModalOpen(false);
